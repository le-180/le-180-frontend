    ...
    return {
        ...
        load: async function (this: Loader, { store, meta, logger }) {
            ...
            try {
                ...
                const schema = typeof schemaOrFn === 'function' ? await schemaOrFn() : schemaOrFn;
                ...
                logger.error(`Error loading Strapi content: ${(error as Error).message}`);
                throw error;
            }
        },

        schema: async () => {
            const data = await fetchFromStrapi(`/get-strapi-schema/schema/${contentType}`);
            if (!data?.attributes) {
